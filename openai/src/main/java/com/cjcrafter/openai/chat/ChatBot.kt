...
class ChatBot @JvmOverloads constructor(
    ...
    fun streamResponse(
        request: ChatRequest,
        onResponse: (ChatResponseChunk) -> Unit, // use Consumer instead of Kotlin for better Java syntax
        onFailure: (IOException) -> Unit = { it.printStackTrace() }
    ) {
